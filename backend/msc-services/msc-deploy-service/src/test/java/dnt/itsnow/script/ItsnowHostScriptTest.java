...
public class ItsnowHostScriptTest {
    ...
    public void testCreateHost() throws Exception {
        ...
        expectLastCall().once();
        expect(repository.findById(host.getId())).andReturn(host);

        repository.updateStatus(isA(ItsnowHost.class));
        expectLastCall().once();
        ...
        manager.create(host);
        String invocationId = manager.getInvocationId(host.getId());
