...
/**
 * <h1> The itsnow rest template</h1>
 */
public class ItsnowRestTemplate implements RestOperations {
    ...
    protected <T> T withCsrf(Callback<T> callback) {
        if (configuration.getCsrf() == null) {
            ...
            ResponseEntity<DefaultCsrfToken> response = getForEntity("/security/csrf", DefaultCsrfToken.class, entity);
