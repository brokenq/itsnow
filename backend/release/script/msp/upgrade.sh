...
#   2. find all msp apps in /opt/itsnow/
#   3.   extract download zip to /opt/itsnow/msp-0.2.1@1(if exists add suffix: .1/.2)
#   4.   create temp dir and copy exist config file to it(/opt/itsnow/lastest_msp)
...
file_download=msp-$version.zip

if [[ "$version" =~ SNAPSHOT$ ]]; then
  target="http://$CI/guestAuth/repository/download/Itsnow_Daily_Build_MSP/.lastFinished/$file_download"
else
  target="http://$CI/guestAuth/repository/download/Itsnow_Sprint_Build_MSP/.lastFinished/$file_download"
fi
...
echo "Step 2 find all msp apps in $itsnow_dir"
cd $itsnow_dir
...
do
  ...
  if [[ $type =~ msp$ ]]; then
    ...
    echo "MSP upgraded from $old_version to $version, link to $folder"
    ...
    if [ $? -eq 0 ]; then
      echo "And new $app msp started"
    else
      echo "But new $app msp failed to start"
    fi  

    echo "=====================upgrade msp:$file finish!=============================================="
