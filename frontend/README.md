# [ItsNow](http://joshdmiller.github.com/ng-boilerplate) [![Build Status](https://api.travis-ci.org/ngbp/ngbp.png?branch=v0.3.2-release)](https://travis-ci.org/ngbp/ngbp)
...
`ItsNow` is designed to make life easy by providing a basic framework
...
ItsNow also comes prepackaged with the most popular design frameworks
...
The principal goal of `ItsNow` is to set projects up for long-term
success.  So `ItsNow` tries to follow best practices everywhere it can.
These are:
...
- It should be responsive to evidence. Community feedback is therefore crucial
  to the success of `ItsNow`.

But `ItsNow` is not an example of an AngularJS app: this is a
...
getting `ItsNow` up and running. Though `ItsNow` is really simple
...
`ItsNow` uses [Grunt](http://gruntjs.org) as its build system, so
...
There are many Bower packages used by `ItsNow`, like Twitter Bootstrap
...
Technically, `ItsNow` is now ready to go.
...
`ItsNow`. What follows in this section is a quick introduction to the
...
`ItsNow` also includes [Live Reload](http://livereload.com/), so you no
...
default with `ItsNow`) you need to tell Live Reload to allow it. Go to
