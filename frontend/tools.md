# The Tools Used in `ItsNow`

## Introduction

`ItsNow` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `ItsNow` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`ItsNow` uses git to manage its codebase. While in theory you don't have
to use Git once you download `ItsNow`, this project makes the assumption
...
While `ItsNow` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `ItsNow` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `ItsNow`; Grunt is
...
to get started with `ItsNow`. But for those curious or looking to go a
...
Grunt is the engine behind `ItsNow`. It's the magic that makes it move.
...
`ItsNow` comes with a `bower.json` file that looks something like this:
...
in `ItsNow` by default in there to see what I mean.
