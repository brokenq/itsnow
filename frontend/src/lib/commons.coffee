...
#                     ]
      formatSelectDatasWithGroup: (srcDatas, displayField, customId, groupType)->
        ...
        for data in srcDatas
          ...
          groupDatas.push groupData[data[groupType]]
          groupData[data[groupType]].push data

        for data in groupDatas
          datas.push {id: "", text: "#{data[groupType]}", disabled: true, children: @formatSelectDatas(data, displayField, customId)}
        return datas
